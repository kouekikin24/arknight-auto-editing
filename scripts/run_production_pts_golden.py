...
def _probe_json(ffprobe: Path, source: Path) -> dict[str, Any]:
    completed = subprocess.run(
        [
            str(ffprobe),
            "-hide_banner",
    ...
    if completed.returncode != 0:
        raise RuntimeError(
            f"ffprobe output verification failed: {completed.stderr.strip()}"
        )
    value = json.loads(completed.stdout)
    if not isinstance(value, dict):
        raise RuntimeError("ffprobe output is not a JSON object")
    return value
...
def run_golden(
    ...
    *,
    ...
    if not ffmpeg.is_file() or not ffprobe.is_file():
        raise FileNotFoundError("explicit FFmpeg pair is incomplete")
    if output_dir.exists():
        raise FileExistsError(f"golden output is write-once: {output_dir}")
    output_dir.mkdir(parents=True, exist_ok=False)

    media = probe_media(source, ffprobe_path=ffprobe, ffmpeg_path=ffmpeg)
    ...
    ffprobe_output = _probe_json(ffprobe, output_path)

    write_once(output_dir / "output_pts_oracle.json", output_oracle)
    write_once(output_dir / "output_ffprobe.json", ffprobe_output)
    manifest = {
        "schema_version": 1,
        ...
        "tools": {
            ...
            "ffprobe": {
                **file_record(ffprobe, repo_root=Path(__file__).resolve().parents[1]),
                "version_line": _version_line(ffprobe),
            },
        },
        ...
        "output_evidence": {
            ...
            "ffprobe": file_record(output_dir / "output_ffprobe.json", repo_root=output_dir),
        },
        "status": "PASS",
