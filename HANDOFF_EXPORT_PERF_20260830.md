...
注：e2e 用的是同规模**合成**清单（无法从 GUI 进程提取用户真实清单），用户的真实
清单由 GUI 复跑验收（内置容器后检会在导出尾段自动跑）。
...
1. **用户 GUI 真实验证**（唯一未闭环项；⚠️ 代码已在 9/01 更新，**必须重启 GUI** 加载新代码）：
   源码启动 `python main.py`（打包 exe 是旧代码），加载样本 4，用平时的剪法 +
   勾"保留音频"导出。预期：**不再弹"超过音频安全上限 80"对话框**（认证路线已豁免），
   成功、带声音、约 7-10 分钟（13,619 段 = 视频 7 批 + 音频 137 批；同规模 e2e 实测
   7.4 分钟，见 §2.5）。混音失败会提示降级为无声视频，不删产物。
