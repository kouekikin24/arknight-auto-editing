...
> **2026-09-01 后续会话见 `HANDOFF_PR_STRATEGY_20260901.md`**（PR 可行性/上游景观/下一步决策）。
...
**阶段三已完成并经 2026-09-01 补丁扩展到真实剪法规模：用户实际剪法是逐帧级精剪
（样本 4 = 13,619 保留段），触发两道过时段数守卫 + 一道 FFmpeg 表达式栈溢出真墙，
VFR 视频侧也改为分批寻址；13,619 段同规模 e2e 全过（7.4 分钟带音频、PTS 对认证表
逐帧一致 20428/20428），pytest 421+90 全绿。**
此前成绩：样本 4（2623 段清单）从"3 小时超时失败"变为 12.7 分钟成功导出、与单趟基准
逐帧一致；样本 2/3 回归通过。详见 §2.4（阶段三）与 §2.5（9/01 补丁）。
**GUI 真实验证已于 2026-09-01 闭环**：真实剪法导出 4.1_clipped.mp4 全过（§2.5 末尾）。
本专项到此全部完成。
...
**未做/可选**：并行编码（编码 2-4 倍加速，优先级低）；CFR 分支千段同样有病
（调研 B），留给未来 CFR 源出现时再处理；真实 5.6 分钟剪法重建（对齐 v3）放弃
（内容对齐被 imageio 0.74% 拉伸破坏）。

### 2.5 阶段三补丁（2026-09-01）：真实剪法 13,619 段触发的新墙 → VFR 视频也分批

GUI 真实验证第一跑就暴露盲区：用户的实际剪法是**逐帧级精剪**——样本 4 保留
**13,619 段**（~1.5 帧/段、保留 ~5.7 分钟），远超此前测试用的 8/22 旧清单
（2,623 段）。两道上古守卫（均 8/16 架构期引入）把它挡在门外，随后实测又发现
第三道真墙：

1. **音频预检 80 段**（`_MAX_FILTER_EXPORT_RANGES_AUDIO`，7af778a）：GUI 弹
   "保留段 13619 个，超过音频安全上限 80"。认证 PTS 路线的音频已分批，段数不再是
   风险 → `inspect_export_plan` 新增 `pts_certified` 参数，认证路线跳过该确认；
   legacy 路线的 80 段精混音守卫保留不变。
2. **PTS 视频 4000 段硬上限**（`_MAX_PTS_EXPORT_RANGES`，1735f8d）：初判"过保守"，
   提到 32768 后**立刻被实测打脸**——13,619 项的平铺 select/setpts 表达式让 ffmpeg
   以 **0xC00000FD（表达式求值栈溢出）秒崩**。探针确认 4000 项安全（墙在
   4000~13619 之间）——这道旧上限其实一直是对的，别删。
3. **修法 = VFR 视频也分批**（回到调研原推荐的统一分批）：
   `_export_pts_video_batched`——每 2000 段一批（2623 已 e2e 验证、4000 探针通过，
   2× 余量），`-ss` 寻址 + `-copyts` + 批内平铺 select/setpts + `-frames:v` 批帧数上限
   （消掉原型扫到 EOF 的浪费）→ concat demuxer + 精确 `duration`（批跨度 = 批内保留
   tick 和，拼接轴 = 全局压缩轴）→ 仅末批带 tpad 克隆哨兵 + setts 修复（批内局部坐标，
   concat 位移后与全局逐 tick 对齐）。原单趟代码提取为
   `_export_pts_video_single_pass`，两路共用下游（音频分批/混流/容器后检）。
   CFR 分支保留 4000 硬上限（平方病未治，无 CFR 样本）。
4. 元数据新增 `pts_video_consumer`（batched_seek_concat / single_pass）。

**验证**：单测 +6（路由/分批数学/预检），pytest **421 passed + 90 subtests** 全绿；
13,619 段合成清单（对照认证表、与用户剪法同规模同形态，脚本
`.cache/e2e_13619_real_scale.py`）走生产 `export_pts_schedule` 全程 **7.4 分钟**完成：
20429 帧（含 1 克隆哨兵）、输出 PTS 与认证表逐帧一致 20428/20428、音频 137 批 PCM
路线长度差 +640 样本（容差 ±2048）、非静音（峰值 0.123）。

**GUI 真实验证（2026-09-01，闭环）**：用户用真实剪法在新代码 GUI 上导出
`D:\qq下载\920\4.1_clipped.mp4`（1.06GB）——全程约 8 分钟（h264_nvenc），
不再弹音频上限对话框。产物独立复验全过：20,185 帧、336.4s（与用户预期的 5.6 分钟
一致）、1920×**1080**（无 imageio 拉伸）、PTS 全程单调、容器声明时长 = 末帧结束位
（地雷修复在真机生效）、音轨 AAC 48kHz 336.4s 与视频等长、非静音（峰值 0.15）。
注：GPU 占用低是预期——瓶颈在 CPU 解码+滤镜，编码只占零头（详见会话记录）。

### 2.6 编码器调研 + B 帧解禁（2026-09-01 晚）

**调研（成熟项目对照）**：OBS 源码默认值（cqp=20、multipass=qres、lookahead 开、
AQ 开、**bf=2**、tune=hq、p5）；HandBrake 1080p 推荐 RF 20-24（我们 crf22 正中）；
FFmpeg 官方（"用最慢且你能等的预设"、crf 17-28 为合理区间）。
**实测量化**（30s 动作切片 + SSIM 对照）：B 帧是唯一实质杠杆（nvenc -13%、x264 -14%
同清晰度）；multipass/lookahead/AQ/更贵预设在 Ada 代合计 ~1.5%（噪声级）。
...
13,619 段 e2e PASS 且**提速 7.4→5.9 分钟**。

**音频码率结论**：原片音轨就是 128k AAC（实测）——成片再编码超过源码率无意义
（有损天花板不可恢复）。已实施**对齐策略**（owner 拍板）：`_audio_bitrate_args`
探测源片音频码率并传给末代 AAC（探测失败回退 128k，钳位到 [16k, 320k]——
防止 PCM 等病态源报出 AAC 编码器拒绝的码率导致导出失败），作用于认证 PTS 路线的
单趟图内与分批混流两处；legacy 路径未动。实测样本 3 产物音轨 ≈134k、样本 2 分批
混流路线 ≈111k（目标均为 128k，AAC 非严格 CBR）。无损中间件仍保证批边界样本级精确。
...
1. ~~用户 GUI 真实验证~~ **已于 2026-09-01 闭环**（§2.5 末尾：4.1_clipped.mp4 全过）。
   顺带可删 `D:\qq下载\920\4_clipped.mp4`（v26.7.15-fix 留下的无声半成品，417MB）。
