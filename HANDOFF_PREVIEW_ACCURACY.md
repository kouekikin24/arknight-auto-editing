...
### 8.5 待办快照
- 唯一卡点仍是 **用户重启验收 → 提交**（改动集=§5 的 8 文件，测试 409+90 绿）。
- **【最新主线】PyAV 整合任务**（ffprobe/ffmpeg→PyAV，导出除外）已批准、未动工，
  完整方案与实施清单见 **`HANDOFF_PYAV_CONSOLIDATION.md`**（owner 已定：
  oracle 留 ffmpeg / 分析解码迁 PyAV+一致性验证 / av 降到 13.x 对齐 7.1）。
